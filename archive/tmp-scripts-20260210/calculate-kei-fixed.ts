...
// 1990年10月3日 12:00生まれ（仮定）
const birthDate = new Date('1990-10-03T12:00:00');
const gender: 'male' = 'male';

console.log('=================================================');
console.log('藤堂慧（1990年10月3日生まれ）命式計算結果');
console.log('=================================================\n');
...
// 十大主星チェック
console.log('\n--- 十大主星チェック ---');
const twelveStars = result.yangSen.twelveStars;
const stars = Object.values(twelveStars);

console.log('配置:');
Object.entries(twelveStars).forEach(([direction, starName]) => {
  console.log(`  ${direction}: ${starName}`);
});

// 禄存星の有無
const hasRokuso = Object.values(twelveStars).includes('禄存星');
console.log(`\n禄存星: ${hasRokuso ? '✓ あり' : '✗ なし'}`);

// 鳳閣星の有無
const hasHoukaku = Object.values(twelveStars).includes('鳳閣星');
console.log(`鳳閣星: ${hasHoukaku ? '✓ あり' : '✗ なし'}`);

// 調舒星の有無
const hasChoujo = Object.values(twelveStars).includes('調舒星');
console.log(`調舒星: ${hasChoujo ? '✓ あり' : '✗ なし'}`);

// 司禄星の有無
const hasShiroku = Object.values(twelveStars).includes('司禄星');
console.log(`司禄星: ${hasShiroku ? '✓ あり' : '✗ なし'}`);
...
const checks = {
  ...
  '調舒星: あり': hasChoujo,
  '司禄星: あり': hasShiroku,
  '天中殺: 巳午': dayBranch === '子' || dayBranch === '丑'
};
