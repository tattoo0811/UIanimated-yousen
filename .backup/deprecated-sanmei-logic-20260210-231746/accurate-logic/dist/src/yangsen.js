...
function calculateYangSen(bazi, date) {
    ...
    // 右手: 日干 × 日支蔵干（YANGSEN_HIDDEN_STEMS使用）
    const dayBranchHiddenStem = YANGSEN_HIDDEN_STEMS[bazi.day.branch - 1];
    const dayBranchHiddenIdx = constants_1.STEM_TO_INDEX[dayBranchHiddenStem];
    ...
    const monthBranchHiddenIdx = constants_1.STEM_TO_INDEX[monthBranchHiddenStem];
    const chest = getTenGreatStar(dayStemIdx, monthBranchHiddenIdx);
    // 左手: 日干 × 年干（朱学院検証データに基づき年干を使用）
