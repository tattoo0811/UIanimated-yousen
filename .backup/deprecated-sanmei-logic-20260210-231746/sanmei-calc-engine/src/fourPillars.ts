 ...
 * '甲' = 0, '乙' = 1, ... '癸' = 9
 */
function getStemIndex(stem: Stem): number {
 ...
 * '子' = 0, '丑' = 1, ... '亥' = 11
 */
function getBranchIndex(branch: Branch): number {
 ...
 * インデックスから天干を取得
 */
function getStemByIndex(index: number): Stem {
  return STEMS[Math.abs(index) % 10];
}

/**
 * インデックスから地支を取得
 */
function getBranchByIndex(index: number): Branch {
  return BRANCHES[Math.abs(index) % 12];
}
...
function getLichunDate(year: number): { month: number; day: number } {
  ...
  // テーブルから取得を試みる
  const termData = getSolarTermData(year);
  return {
    month: 2,
 ...
 */
function getChineseMonth(year: number, month: number, day: number): number {
  ...
  const termData = getSolarTermData(termYear);
...
function calculateMonthPillar(
  ...
  const wuHuBase = WU_HU_DUN[yearStemIndex % 5];
...
function calculateHourPillar(dayStem: Stem, hour: number): Pillar {
  ...
  const wuShuBase = WU_SHU_DUN[dayStemIndex % 5];
...
function createPillar(
  ...
  return {
    ...
    element: STEM_ELEMENTS[stem], // 天干の五行
    yinYang: STEM_YIN_YANG[stem], // 天干の陰陽
