...
exports.calculateTenchusatsu = calculateTenchusatsu;
exports.isBranchInVoid = isBranchInVoid;
exports.isKanshiInVoid = isKanshiInVoid;
exports.formatTenchusatsu = formatTenchusatsu;
 ...
 * @returns グループ番号 (0-5)
 */
function calculateGroupIndex(kanshiIndex) {
 ...
 */
function getVoidBranches(groupIndex) {
 ...
 * @returns 天中殺のオブジェクト
 */
function calculateTenchusatsu(dayKanshiIndex) {
    // バリデーション
    ...
    // 虚の支の名前を取得（支の名前から）
    const voidBranch1Name = constants_1.BRANCHES[voidInfo.branch1];
    const voidBranch2Name = constants_1.BRANCHES[voidInfo.branch2];
 ...
 * @returns 影響を受ける年の配列
 */
function calculateAffectedYears(groupIndex, indexInGroup) {
    // 天中殺の影響は12年周期
 ...
 * @returns 日本語の説明文
 */
function generateTenchusatsuDescription(tenchusatsuType) {
    const descriptions = {
        '戌亥天中殺': '戌亥の方向（北西）に関する運が低下します。不動産や家族関係、長期計画に影響が出やすい時期です。',
        '申酉天中殺': '申酉の方向（西）に関する運が低下します。対人関係、契約、金銭面に影響が出やすい時期です。',
        '午未天中殺': '午未の方向（南）に関する運が低下します。恋愛、結婚、健康面に影響が出やすい時期です。',
        '辰巳天中殺': '辰巳の方向（南東）に関する運が低下します。仕事、成長、発展に影響が出やすい時期です。',
        '寅卯天中殺': '寅卯の方向（東）に関する運が低下します。新規事業、発進、決断に影響が出やすい時期です。',
        '子丑天中殺': '子丑の方向（北）に関する運が低下します。基盤、安定、内面に影響が出やすい時期です。'
    };
 ...
 */
function isBranchInVoid(branchIndex, tenchusatsu) {
 ...
 */
function isKanshiInVoid(kanshiIndex, tenchusatsu) {
    // 干支のブランチ部分を取得（干支 % 12）
    const branchInKanshi = kanshiIndex % 12;
    return (branchInKanshi === tenchusatsu.branch1 ||
 ...
 * @returns 整形された説明文
 */
function formatTenchusatsu(tenchusatsu) {
    return `
