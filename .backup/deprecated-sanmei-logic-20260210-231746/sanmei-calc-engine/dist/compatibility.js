...
exports.generateCompatibilityAdvice = generateCompatibilityAdvice;
exports.formatCompatibilityResult = formatCompatibilityResult;
 ...
 * @returns スコアと説明
 */
function calculateBaseElementScore(person1DayStem, person2DayStem) {
    const element1 = (0, fiveElements_1.getElement)(person1DayStem);
    const element2 = (0, fiveElements_1.getElement)(person2DayStem);
    const relationship = (0, fiveElements_1.getRelationship)(element1, element2);
    let score = 0;
    let explanation = '';
    switch (relationship) {
        case 'same':
 ...
 * @returns スコアと説明
 */
function calculateKangoBonus(person1DayStemIndex, person2DayStemIndex) {
    const kango = (0, fiveElements_1.checkKango)(person1DayStemIndex, person2DayStemIndex);
    if (kango.isKango) {
        return {
            score: 20,
 ...
 * @returns スコアと説明
 */
function calculateBranchScore(person1DayBranch, person2DayBranch) {
    // 地支は文字列として渡される想定
    const branch1 = person1DayBranch;
    const branch2 = person2DayBranch;
    // 支合の確認
    const partnership = (0, fiveElements_1.checkBranchPartnership)(branch1, branch2);
    ...
    // 支冲の確認
    if ((0, fiveElements_1.checkBranchClash)(branch1, branch2)) {
        return {
            score: 0, // 最小値に設定
 ...
 * @returns スコアと説明
 */
function calculateYearHarmonyScore(person1YearStemIndex, person2YearStemIndex, person1YearBranch, person2YearBranch) {
    let score = 0;
    const details = [];
    // 年干の干合
    const yearKango = (0, fiveElements_1.checkKango)(person1YearStemIndex, person2YearStemIndex);
    if (yearKango.isKango) {
        score += 10;
        details.push('年干の干合あり');
    }
    // 年支の支合
    const branch1 = person1YearBranch;
    const branch2 = person2YearBranch;
    const yearPartnership = (0, fiveElements_1.checkBranchPartnership)(branch1, branch2);
    if (yearPartnership.isPartnership) {
        score += 5;
        details.push('年支の支合あり');
    }
    const explanation = details.length > 0
        ? `年柱調和: ${details.join(', ')}`
        : '年柱に特殊な関係なし';
    return { score, explanation };
}
 ...
 * @returns スコアと説明
 */
function calculateElementBalanceScore(person1Stems, person1Branches, person2Stems, person2Branches) {
    // 簡略版: 各人物の支配的な要素を比較
    const countElements = (stems, branches) => {
        const counts = {
            '木': 0,
            '火': 0,
            '土': 0,
        ...
        stems.forEach((stem) => {
            const element = (0, fiveElements_1.getElement)(stem);
            counts[element]++;
        });
        branches.forEach((branch) => {
            const element = (0, fiveElements_1.getBranchElement)(constants_1.BRANCHES.indexOf(branch));
            counts[element]++;
        });
        return counts;
    };
...
function calculateCompatibility(person1, person2) {
    // 干支をインデックスに変換
    const person1DayStemIdx = constants_1.STEMS.indexOf(person1.dayStem);
    const person2DayStemIdx = constants_1.STEMS.indexOf(person2.dayStem);
    const person1YearStemIdx = constants_1.STEMS.indexOf(person1.yearStem);
    const person2YearStemIdx = constants_1.STEMS.indexOf(person2.yearStem);
    const person1MonthStemIdx = constants_1.STEMS.indexOf(person1.monthStem);
    const person2MonthStemIdx = constants_1.STEMS.indexOf(person2.monthStem);
    const details = [];
    // 1. 日干の五行関係スコア
    const baseScore = calculateBaseElementScore(person1DayStemIdx, person2DayStemIdx);
    ...
    // 3. 地支相性スコア
    const branchScore = calculateBranchScore(person1.dayBranch, person2.dayBranch);
    details.push(branchScore.explanation);
    // 4. 年柱の調和スコア
    const yearScore = calculateYearHarmonyScore(person1YearStemIdx, person2YearStemIdx, person1.yearBranch, person2.yearBranch);
    ...
    const elementBalance = calculateElementBalanceScore(allStems1, allBranches1, allStems2, allBranches2);
 ...
 */
function generateCompatibilityAdvice(result) {
    const baseAdvice = {
        excellent: '最高の相性です。二人は互いに補い合い、自然な調和が生まれやすいでしょう。深い信頼と共感に基づく関係が築けます。',
        ...
        incompatible: '相性に大きな課題があります。二人の基本的な価値観が異なる可能性があります。強い意志と努力によって関係を築くことは可能ですが、挑戦的になるでしょう。',
    };
