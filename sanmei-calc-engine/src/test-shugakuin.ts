 ...
 * 検証対象: 月柱計算の修正確認
 */
import { calculateFourPillars, internalFunctions } from './fourPillars';

const { getChineseMonth } = internalFunctions;
...
for (const tc of testCases) {
  ...
  const pillars = calculateFourPillars(tc.year, tc.month, tc.day, tc.hour);
...
// 白露(Sep 8)の前後
const beforeBailu = calculateFourPillars(1995, 9, 7, 12); // 白露前 → 申月(7)
const afterBailu = calculateFourPillars(1995, 9, 8, 12);  // 白露当日 → 酉月(8)
const afterBailu2 = calculateFourPillars(1995, 9, 9, 12); // 白露後 → 酉月(8)
...
if (bOk1 && bOk2 && bOk3) passCount += 3; else failCount += 3;

// 立春(Feb 4)の前後
const beforeLichun = calculateFourPillars(1995, 2, 3, 12); // 立春前 → 丑月(12)
const afterLichun = calculateFourPillars(1995, 2, 4, 12);  // 立春当日 → 寅月(1)
...
if (lOk1 && lOk2) passCount += 2; else failCount += 2;

// 1月テスト（小寒前後）
const jan3 = calculateFourPillars(1995, 1, 3, 12);  // 小寒前 → 子月(11)
const jan6 = calculateFourPillars(1995, 1, 6, 12);  // 小寒後 → 丑月(12)
...
if (jOk1 && jOk2) passCount += 2; else failCount += 2;
