...
exports.getTsuhenExplanation = getTsuhenExplanation;
...
function getStemElement(stem) {
    return constants_1.ELEMENT_MAP[constants_1.STEMS[stem]];
}
/**
 * 天干の陰陽を取得
 * @param stem - 天干のインデックス (0-9)
...
function getStemPolarity(stem) {
    return constants_1.YIN_YANG_MAP[constants_1.STEMS[stem]];
}
/**
 * 5行間の相生関係をチェック
 * (木→火→土→金→水→木)
 ...
 */
function checksGeneration(source, target) {
    return constants_1.GENERATION_RELATIONS[source] === target;
}
/**
 * 5行間の相剋（制御）関係をチェック
 * (木→土→水→火→金→木)
 ...
 */
function checksControl(source, target) {
    return constants_1.CONTROL_RELATIONS[source] === target;
}
/**
 * 日干と対象天干から通変星を導出
 *
 ...
 * @returns 通変星の種類
 */
function deriveTsuhenSei(dayStem, targetStem) {
 ...
 * @returns 十大主星
 */
function mapTsuhenToTenStar(tsuhensei) {
    const mapping = {
        '比肩': '貫索星',
        '劫財': '石門星',
        '食神': '鳳閣星',
 ...
 * @returns 十大主星チャート
 */
function calculateTenStars(fourPillars) {
    ...
    const dayStemIdx = constants_1.STEMS.indexOf(dayStem);
    const yearStemIdx = constants_1.STEMS.indexOf(yearStem);
    const monthStemIdx = constants_1.STEMS.indexOf(monthStem);
    // 蔵干情報から隠れた干を取得
    const monthHiddenStem = constants_1.HIDDEN_STEMS[monthBranch].main;
    const dayHiddenStem = constants_1.HIDDEN_STEMS[dayBranch].main;
    const yearHiddenStem = constants_1.HIDDEN_STEMS[yearBranch].main;
    const monthHiddenStemIdx = constants_1.STEMS.indexOf(monthHiddenStem);
    const dayHiddenStemIdx = constants_1.STEMS.indexOf(dayHiddenStem);
    const yearHiddenStemIdx = constants_1.STEMS.indexOf(yearHiddenStem);
    return {
        // 頭（Head）：年干 vs 日干
        head: {
 ...
 * @returns 日本語の関係説明
 */
function getTsuhenExplanation(dayStemValue, targetStemValue) {
    const dayStemIdx = constants_1.STEMS.indexOf(dayStemValue);
    const targetStemIdx = constants_1.STEMS.indexOf(targetStemValue);
