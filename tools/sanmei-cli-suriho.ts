...
interface SurihoEnergy {
    totalEnergy: number;        // 総エネルギー数（64〜343、平均200）
    haikiko: number;            // 排気口（バランス値、1〜10、理想7-8）
    fiveElements: FiveElementEnergy;  // 五行別エネルギー
    ganCount: GanCount;         // 十干別個数
    details: {
        [gan: string]: {
            score: number;      // 各十干の合計点数
            count: number;      // 各十干の個数
        }
    };
}
...
interface SanmeiResult {
    ...
    suriho: SurihoEnergy;  // 数理法エネルギー計算結果
}
...
const calculateSurihoEnergy = (insen: Insen, dayGan: string): SurihoEnergy => {
    ...
    return {
        totalEnergy,
        haikiko: Math.round(haikiko * 10) / 10,  // 小数点第1位まで
        fiveElements,
        ganCount,
        details
    };
