...
const calculateAll = (year, month, day, gender) => {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        // 位相法チェック (大運 vs 日/月/年)
        const westIsoho = getIsohoList(tg, ts, dGan, dShi); // vs 日柱
        const centerIsoho = getIsohoList(tg, ts, mGan, mShi); // vs 月柱
        const eastIsoho = getIsohoList(tg, ts, yGan, yShi); // vs 年柱
...
export default function SanmeiFullApp() {
    const [date, setDate] = useState("1980-05-06"); // デフォルトを例題に合わせる
