...
// 数理法エネルギー点数表（修正版 v6）
// 2026-02-15: ユーザー指摘に基づき修正（特に癸-巳=3など）
...
export default function SanmeiFullApp() {
            ...
            <div className="max-w-4xl mx-auto flex items-center justify-between" >
                <div className="flex items-center gap-2" >
                    <Moon className="w-5 h-5 text-yellow-400" />
                        <h1 className="text-lg font-bold tracking-widest" > 算命学・数理法・天中殺(v6) </h1>
                            </div>
                            </div>
                            </header>
    ...
    className = {`flex-1 px-4 py-2 rounded border ${gender === 'female' ? 'bg-pink-50 border-pink-300 text-pink-700' : 'bg-white text-stone-400'}`
}
                        >
    女性
    </button>
...
className = {`flex-1 px-4 py-2 rounded border ${gender === 'male' ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white text-stone-400'}`}
                        >
    男性
    </button>
    </div>
