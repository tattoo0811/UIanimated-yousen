...
import type { FiveElements, TwentyEightElementData } from './types';
...
// 位相ID→点数マッピング
export const PHASE_TO_SCORE = [11, 12, 8, 4, 2, 5, 1, 3, 6, 9, 7, 10] as const;
...
export const TWENTY_EIGHT_ELEMENTS_DAY: Record<string, TwentyEightElementData> = {
  '子': {
...
export const TWENTY_EIGHT_ELEMENTS_MONTH: Record<string, TwentyEightElementData> = {
  '子': {
    extra: { stem: '癸', days: 19 },
    main: { stem: '癸' }
  },
...
// 二十八元データ（デフォルトは日支用）
export const TWENTY_EIGHT_ELEMENTS = TWENTY_EIGHT_ELEMENTS_DAY;

// 節入り日データ（毎月の節入り日の概ねの日付）
// 注意: 11月と12月は正確な日付（小雪: 11/22, 大雪: 12/7）
export const SOLAR_TERM_DAYS: Record<number, number> = {
  1: 6,   // 小寒
  2: 4,   // 立春
  3: 6,   // 啓蟄
  4: 5,   // 清明
...
// 地支文字列→インデックス
export const BRANCH_TO_INDEX: Record<string, number> = {
...
// 地支の五行取得
export function getBranchElement(branchIdx: number): number {
  // 寅卯(2,3)→木(0), 巳午(5,6)→火(1), 辰戌丑未(4,10,1,7)→土(2), 申酉(8,9)→金(3), 子亥(0,11)→水(4)
  if (branchIdx === 2 || branchIdx === 3) return 0; // 木
  if (branchIdx === 5 || branchIdx === 6) return 1; // 火
  if (branchIdx === 4 || branchIdx === 7 || branchIdx === 10 || branchIdx === 1) return 2; // 土
  if (branchIdx === 8 || branchIdx === 9) return 3; // 金
  return 4; // 水
}
