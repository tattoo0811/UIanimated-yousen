...
exports.getHiddenStemByDays = getHiddenStemByDays;
exports.getTenGreatStar = getTenGreatStar;
exports.getTwelveGreatStar = getTwelveGreatStar;
exports.calculateYangSen = calculateYangSen;
...
function getDaysFromSolarTerm(date) {
    ...
    const solarTermDay = constants_1.SOLAR_TERM_DAYS[month];
    if (day < solarTermDay) {
        const prevMonth = month === 1 ? 12 : month - 1;
        const prevMonthLastDay = new Date(date.getFullYear(), date.getMonth(), 0).getDate();
        const prevSolarTermDay = constants_1.SOLAR_TERM_DAYS[prevMonth];
 ...
 * @returns 蔵干（天干）
 */
function getHiddenStemByDays(branchStr, daysFromSolarTerm, elementsData = constants_1.TWENTY_EIGHT_ELEMENTS) {
    const data = elementsData[branchStr];
    if (!data)
        throw new Error(`Invalid branch: ${branchStr}`);
    // 余気（初気）チェック
...
const BRANCH_HIDDEN_STEM_BY_MONTH = {
    ...
    '申': ['戊', '丙', '壬', '戊', '丙', '壬', '戊', '丁', '壬', '戊', '丙', '壬'], // 8月目を丁に修正
 ...
 * @returns 十大主星名
 */
function getTenGreatStar(dayStemIdx, targetStemIdx) {
    // 朱学院の正解データに基づく十大主星表
    // 行: 日干（甲乙丙丁戊己庚辛壬癸）
    // 列: 対象天干（甲乙丙丁戊己庚辛壬癸）
    // セル: 十大主星名
 ...
 * @returns 星情報（名前と点数）
 */
function getTwelveGreatStar(dayStemIdx, branchIdx) {
    // 朱学院の正解データに基づく十二大従星表
    // 行: 十二大従星の名前と点数
    // 列: 日干（甲乙丙丁戊己庚辛壬癸）
    // セル: 地支（子丑寅卯辰巳午未申酉戌亥）
    ...
    for (let starIdx = 0; starIdx < 12; starIdx++) {
        ...
        if (cell && cell === constants_1.BRANCHES[branchIdx]) {
            const twelveIdx = tableToTwelveIdx[starIdx];
            return {
                name: constants_1.TWELVE_STARS[twelveIdx],
                score: twelveIdx + 1
            };
        }
    }
 ...
 */
function calculateYangSen(bazi, date) {
    ...
    const monthHiddenStemIdx = constants_1.STEM_TO_INDEX[monthBranchHiddenStem];
    const belly = getTenGreatStar(dayStemIdx, monthHiddenStemIdx);
    // 右手: 日干 × 日支の蔭干（月に基づく蔵干）
    const dayBranchHiddenStem = getBranchHiddenStemByMonth(bazi.day.branchStr, date);
    const dayHiddenStemIdx = constants_1.STEM_TO_INDEX[dayBranchHiddenStem];
    ...
    const yearHiddenStemIdx = constants_1.STEM_TO_INDEX[yearBranchHiddenStem];
